...
double newton_backward_divided_difference_interpolation(double x, const std::vector<double>& data_x, const std::vector<double>& data_y) {
    ...
    // Print the results
    std::cout << "Newton's Backward Divided Difference Interpolation at x=" << x << ": " << result << std::endl;
...
int main() {
    std::vector<double> data_x = {0, 1, 2, 3, 4};
    std::vector<double> data_y = {0, 1, 5, 12, 30};
    double x_to_interpolate = 3.5;

    newton_backward_divided_difference_interpolation(x_to_interpolate, data_x, data_y);
